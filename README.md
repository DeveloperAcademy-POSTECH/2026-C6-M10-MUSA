...
| 한 판 | 시작 구슬0·스태미나100, 생성 비용20, 시간 회복20/3초, 명중 회복5, HP 인원별 800~1400·피해20·180초·결과·Retry |
| 방 생성 개발자 모드 | Host가 방을 만들기 전에 인원별 몬스터 HP·피해·개인 구슬 한도·구슬 크기·시간·생성/회복 값을 임시 조정 |

빌드25의 개발자 설정은 해당 방의 Host 승인 설정으로만 사용하며 저장된 Config 에셋은 바꾸지 않습니다. 자동 검사 범위와 실기기 미실행 항목은 [개발자 모드 검증 기록](docs/validation/DEVELOPER_MODE_20260921.json)에 구분했습니다.
...
- [정면 전투 그래픽 — Figma 에셋과 수동 배치·검증 기록](docs/FIGMA_FRONT_ARTWORK.md)
