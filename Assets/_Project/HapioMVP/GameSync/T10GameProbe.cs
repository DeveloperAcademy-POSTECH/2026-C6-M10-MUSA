...
namespace C6.Prototype.GameSync
{
    ...
    public sealed class T10GameProbe : MonoBehaviour
    {
#if DEVELOPMENT_BUILD && UNITY_STANDALONE && !UNITY_EDITOR
        [RuntimeInitializeOnLoadMethod(RuntimeInitializeLoadType.AfterSceneLoad)]
        private static void Attach()
        {
            if (Array.IndexOf(Environment.GetCommandLineArgs(), "-c6T10BNetworkProbeDirectory") < 0) return;
            var owner = FindAnyObjectByType<T09BattleController>();
            if (owner != null && owner.GetComponent<T10GameProbe>() == null) owner.gameObject.AddComponent<T10GameProbe>();
        }

        private T09BattleController controller;
        private T10GameSession game;
        private double nextClockSample;
        private int droppedReceipts, duplicatePackets, oldRoundPackets;
        private ResourceRequestPacket oldGeneration;
        private readonly List<ClockSample> clocks = new List<ClockSample>();
        private void Update()
        {
            if(game?.Snapshot==null||Time.realtimeSinceStartupAsDouble<nextClockSample)return;
            nextClockSample=Time.realtimeSinceStartupAsDouble+.5;
            clocks.Add(new ClockSample{utc=DateTime.UtcNow.ToString("O"),remaining=game.DisplayRemaining,round=game.Snapshot.roundId,phase=game.Snapshot.battle.phase});
        }
        private void DropOneResourceReceipt()
        {
            var session=controller.Resource;
            var method=typeof(ResourceSession).GetMethod("ReceiveReply",BindingFlags.NonPublic|BindingFlags.Instance);
            var original=(CustomMessagingManager.HandleNamedMessageDelegate)Delegate.CreateDelegate(typeof(CustomMessagingManager.HandleNamedMessageDelegate),session,method);
            var messages=controller.Attack.Connection.OwnedManager.CustomMessagingManager;
            messages.RegisterNamedMessageHandler("C6.T07.Reply.v1",(sender,reader)=>
            {
                if(droppedReceipts==0){droppedReceipts++;Debug.Log("C6_T10B_EXPLICIT_FAULT droppedResourceReply=1");return;}
                original(sender,reader);
            });
        }
        ...
        private IEnumerator Start()
        {
            ...
            if (Array.IndexOf(args, "-c6T10BNetworkProbeDirectory") < 0) yield break;
            output = Arg(args, "-c6T10BNetworkProbeDirectory", null);
            role = Arg(args, "-c6T10BNetworkRole", "host");
            port = Arg(args, "-c6T10BNetworkPort", "25125");
            ...
            try
            {
                ...
                controller = GetComponent<T09BattleController>(); game = GetComponent<T10GameSession>();
                if(role=="client")game.DevelopmentHoldInitialAck=true;
            ...
            report.proofs=game.Proofs.ToArray(); report.clocks=clocks.ToArray(); report.droppedReceipts=droppedReceipts;report.duplicatePackets=duplicatePackets;report.oldRoundPackets=oldRoundPackets; report.gameError=game.Error; report.initialAcks=game.InitialAcks;
            ...
            Debug.Log($"C6_T10B_NETWORK_PROBE_COMPLETE status={report.status} role={role} error={error}");
            Application.logMessageReceived -= ObserveLog;
            ...
            if (Array.IndexOf(args, "-c6T10BNetworkProbeQuit") >= 0) Application.Quit(error == null ? 0 : 1);
        }
        ...
        private IEnumerator Run()
        {
            ...
            Require(game != null && !game.Attached, "Game attached before Lobby agreement.");
            bool started = role == "host" ? game.Lobby.CreateRoom("C6 T10B", port) : game.Lobby.JoinDirect("127.0.0.1",port);
            Require(started, "Lobby connection refused.");
            if(role=="host")
            {
                yield return WaitFor(()=>game.Lobby.Connected,20,"Host Lobby did not open.");
                Require(!game.Attached&&!controller.RequestHostStart()&&!controller.RequestGenerate(),"Single Host bypassed Lobby.");
                File.WriteAllText(Path.Combine(output,"host-lobby-ready.json"),JsonUtility.ToJson(game.Lobby.Snapshot,true));
            }
            yield return WaitFor(()=>game.Lobby.CanReady,30,"Config initial ACK did not unlock Ready.");
            Require(game.Lobby.ToggleReady(),"Ready request rejected.");
            if(role=="host")
            {
                yield return WaitFor(()=>game.Lobby.CanStart,35,"Both Ready missing.");
                Require(game.Lobby.StartMatch(),"Lobby Host Start failed.");
            }
            yield return WaitFor(()=>game.Snapshot!=null&&game.Snapshot.battle.phase=="Ready",20,"Initial game snapshot missing.");
            report.localPlayer=controller.Attack.LocalPlayerId;
            Require(!game.InitialConfirmed&&!controller.Battle.CanAct&&!controller.RequestHostStart()&&!controller.RequestGenerate(),
                "Game started before initial state ACK.");
            yield return Capture("initial-state-held");
            if(role=="client")
            {
                yield return new WaitForSecondsRealtime(2);
                Require(game.Snapshot.battle.phase=="Ready"&&game.Snapshot.battle.remaining==180&&controller.Views.Count==0,
                    "Missing initial ACK allowed timer or generation.");
                report.missingInitialAckBlocked=true; game.DevelopmentHoldInitialAck=false;
            }
            yield return WaitFor(()=>controller.Battle.Phase==BattlePhase.Playing&&controller.Resource.Snapshot.playing&&game.InitialConfirmed,
                20,"Confirmed Lobby/game states did not start.");
            Require(controller.Battle.Snapshot.roundId==1&&controller.Views.Count==0&&controller.Resource.LocalPlayer.stamina==100,
                "Start changed approved round or initial state.");
            ...
            if(role=="client")DropOneResourceReceipt();
            for (int index = 0; index < 5; index++) yield return Generate();
            if(role=="client")Require(droppedReceipts==1&&controller.Resource.LocalPlayer.generatedTotal==5,
                "Missing receipt query duplicated generation or did not complete.");
            ...
            if(role=="host") { yield return new WaitForSecondsRealtime(2); controller.RetryBattle(); }
            yield return WaitFor(()=>game.Snapshot!=null&&game.Snapshot.roundId==2&&game.Snapshot.battle.phase=="Ready",15,"Reset not synchronized.");
            Require(controller.Views.Count==0&&controller.Resource.LocalPlayer.stamina==100&&game.Snapshot.attack.hp==100
                &&game.Snapshot.battle.remaining==180,"Reset was not empty/full.");
            yield return Capture("retry-empty-ready");
            if(role=="host")
            {
                yield return WaitFor(()=>game.CanStart,10,"Reset initial ACK missing.");
                yield return new WaitForSecondsRealtime(1);Require(controller.RequestHostStart(),"Prepared second round refused.");
            }
            yield return WaitFor(()=>game.Snapshot.battle.phase=="Playing",12,"Second round missing.");
            Require(game.Snapshot.roundId==2,"Host Start reset the round twice.");
            yield return Generate();
            Require(controller.Resource.LocalPlayer.generatedTotal==1&&controller.Views.Count==1,"First post-reset Generate did not produce exactly one.");
            if(role=="client")
            {
                SendGenerationPacket(oldGeneration);oldRoundPackets++;
                yield return new WaitForSecondsRealtime(1);
                Require(controller.Resource.LocalPlayer.generatedTotal==1&&controller.Views.Count==1,
                    "Previous round packet changed the new battle.");
            }
            yield return Capture("retry-first-one");
            yield return WaitFor(()=>game.Snapshot.battle.phase=="Defeat",190,"Actual180-second second round did not end.");
            Require(game.Snapshot.attack.hp==100&&game.Snapshot.battle.remaining==0&&game.Snapshot.battle.teamHp==0,"Defeat state inconsistent.");
            yield return Capture("actual180-defeat");
            var defeat=Copy(game.Snapshot);yield return new WaitForSecondsRealtime(1.2f);
            Require(game.Snapshot.battle.remaining==0&&game.Snapshot.battle.teamHp==0&&game.Snapshot.attack.hp==100,"Defeat did not freeze.");
            report.finalGame=Copy(game.Snapshot);report.fullDurationDefeat=true;
        ...
        private IEnumerator Generate()
        {
            ...
            if(role=="client"&&game.Snapshot.roundId==1)
            {
                var field=typeof(ResourceSession).GetField("pending",BindingFlags.Instance|BindingFlags.NonPublic);
                var packet=Copy((ResourceRequestPacket)field.GetValue(controller.Resource));
                if(generations.Count==0)oldGeneration=packet;
                if(generations.Count==1){SendGenerationPacket(packet);duplicatePackets++;}
            }
        ...
        private void SendGenerationPacket(ResourceRequestPacket packet)
        {
            using(var writer=ResourceWire.Write(packet))controller.Attack.Connection.OwnedManager.CustomMessagingManager
                .SendNamedMessage("C6.T07.Generate.v1",NetworkManager.ServerClientId,writer,NetworkDelivery.ReliableFragmentedSequenced);
        }
        private IEnumerator FireConfirmed(string id, int expectedHp)
        {
            ...
            var originalView=controller.Views[id];
            ...
            Require(ReferenceEquals(controller.Views[id],originalView)&&Vector2.Distance(controller.GetViewScreenPosition(id),formerBandPoint)<1,
                "Aggregate recreated an existing OrbId view or moved its local drag position.");
        ...
        private IEnumerator Capture(string name)
        {
            ...
            Require(game.Error.Length==0,"Game coordinator error: "+game.Error);
            File.WriteAllText(Path.Combine(output,name+"-game.json"),JsonUtility.ToJson(game.Snapshot,true));
            File.WriteAllText(Path.Combine(output,"progress.json"),JsonUtility.ToJson(new Progress{name=name,round=game.Snapshot?.roundId??0,phase=game.Snapshot?.battle.phase,display=game.DisplayRemaining,observedUtc=DateTime.UtcNow.ToString("O")}));
        ...
        [Serializable] private sealed class ClockSample {public string utc,phase;public uint round;public double remaining;}
        [Serializable] private sealed class Progress {public string name,phase,observedUtc;public uint round;public double display;}
        [Serializable] private sealed class Report
        {
            ...
            public ClockSample[] clocks; public int droppedReceipts,duplicatePackets,oldRoundPackets; public GameSnapshot finalGame; public GameStateProof[] proofs; public string gameError;public int initialAcks;public bool missingInitialAckBlocked,fullDurationDefeat;
