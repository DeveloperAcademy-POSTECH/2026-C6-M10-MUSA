...
namespace C6.Prototype.Combination
{
    ...
    [RequireComponent(typeof(SplitScreenLayout), typeof(DirectConnectionSession), typeof(T08Hud))]
    public sealed class T08CombinationController : MonoBehaviour, IOrbPointerSink
    {
        [SerializeField] private SplitScreenLayout layout;
        [SerializeField] private DirectConnectionSession connection;
        [SerializeField] private T08Hud hud;
        ...
        private string detail = "Generate Raw / drop Yin onto Yang / drag Combined upward";
        public SplitScreenLayout Layout => layout;
        public T08Hud Hud => hud;
        ...
        public void Configure(SplitScreenLayout split, DirectConnectionSession session, T08Hud overlay,
        ...
        private void Awake()
        {
            ...
            viewRoot = new GameObject("T08 Local Orb Views").transform; viewRoot.SetParent(transform, false);
            proxyRoot = new GameObject("T08 Client Projectile Display Only").transform; proxyRoot.SetParent(transform, false);
        }
        private void Start()
        {
            ...
            RefreshHud();
            Debug.Log($"C6_T08_READY mode=NORMAL initialOrbs=0 build=10 device={SystemInfo.deviceModel} os={SystemInfo.operatingSystem} screen={Screen.width}x{Screen.height} safe={Screen.safeArea} staminaStart={layout.Config.StaminaStart} staminaMax={layout.Config.StaminaMax} generateCost={layout.Config.GenerateCost} recoveryPerSecond={layout.Config.StaminaRecoveryPerSecond} hitRecovery={layout.Config.StaminaHitRecovery}");
            Debug.Log("C6_T08_LOCAL_IPV4 addresses=[" + string.Join(",", DirectConnectionSession.GetLocalIPv4Addresses()) + "]");
        }
        ...
        {
            if (!isActiveAndEnabled || resource == null || !resource.Connected || resource.HasPending || pendingCombination != null) return false;
        ...
        public void StartDebugFixtureRound()
        {
            ...
            if (resource.BeginMixedDebugFixtureRound())
            {
                action = "DEBUG_TEST_MODE / mixed Yin + Yang";
                detail = "Yin / Yang Raw only / drag one onto its opposite";
            }
        ...
        private void OnGenerationResolved(ResourceRequestReply reply)
        {
            ...
            Debug.Log($"C6_T08_GENERATE_UI accepted={reply.accepted} duplicate={reply.duplicate} operation={reply.operation} reason={reply.reason} request={reply.requestId} round={reply.roundId}");
            RefreshHud();
        }
        ...
        private void OnStateChanged()
        {
            ...
            {
                ClearViews(); ClearProxies(); pending.Clear(); pendingCombination = null; sequences.Clear(); confirmedUnavailable.Clear();
            ...
            if (sessionKey != state.sessionId || round != state.roundId)
            {
                ...
                ClearViews(); pending.Clear(); pendingCombination = null; sequences.Clear(); confirmedUnavailable.Clear(); gestures = new OrbGestureEngine();
            ...
            foreach (var orb in available)
                ...
                {
                    var view = new GameObject("T08 Orb " + orb.id).AddComponent<OrbView>(); view.transform.SetParent(viewRoot, false);
        ...
        public bool BeginPointer(int pointerId, Vector2 raw, bool overUi)
        {
            ...
            if (began)
            {
                ...
                detail = record.Kind == OrbKind.Raw ? "Drop onto the opposite Yin / Yang" : "Combined: drag upward to attack / no further combination";
                Debug.Log($"C6_T08_POINTER_BEGIN source={(dragIsTouch ? "TOUCH" : pointerId == OrbPointerInput.MousePointerId ? "MOUSE" : "DEBUG")} pointer={pointerId} orb={record.OrbId} round={round}");
            }
        ...
        public void EndPointer(int pointerId, Vector2 raw)
        {
            ...
            if (decision.HasValue) SubmitDecision(decision.Value, touch);
                ...
                && OrbGestureEngine.AttackZone(layout.BottomPixelRect, layout.Config.AttackZoneHeightFraction).Contains(raw))
            {
        ...
        private void OnCombinationResolved(CombinationReply reply)
        {
            ...
            if (reply.accepted)
            {
                ...
                Debug.Log($"C6_T08_COMBINE_UI accepted=true request={reply.requestId} source={reply.sourceOrbId} target={reply.targetOrbId} combined={reply.originalCombined?.id ?? "none"} sourceInput={(held.touch ? "TOUCH" : "DEBUG_OR_MOUSE")} round={round}");
            }
            else
            {
                action = "COMBINATION REJECTED / " + reply.reason;
                detail = "Use two different Raw orbs: one Yin and one Yang";
                Debug.Log($"C6_T08_COMBINE_UI accepted=false request={reply.requestId} reason={reply.reason} round={round}");
            }
        ...
        private void SubmitDecision(OrbGestureDecision decision, bool fromTouch)
        {
            ...
            // Existing attack authority rejects Transfer actions until T11; launch keeps its T06 path.
        ...
        private void OnResolved(AttackRequestReply reply)
        {
            ...
            {
                TouchLaunches++;
                Debug.Log($"C6_T08_TOUCH_LAUNCH count={TouchLaunches} request={reply.requestId} orb={reply.orbId} round={round} owner={attack.LocalPlayerId}");
            }
            bool transfer = inputRequest.request.Kind == OrbActionKind.TransferLeft || inputRequest.request.Kind == OrbActionKind.TransferRight;
            action = (reply.accepted ? "LAUNCH APPROVED" : transfer ? "SWIPE / TRANSFER NOT READY" : "REJECTED") + " / " + reply.reason;
            detail = reply.accepted ? "Same ID / 2D removed / actual Host physics"
                : transfer ? "Transfer is T11 / use a shorter or vertical drop to combine" : "Host confirmed original state; no duplicate spawn";
        ...
        private void RefreshHud()
        {
            ...
            hud.SetControls(connection.CanStart, connection.CanStart,
                resource != null && resource.IsHost && resource.Connected && !resource.HasPending && pendingCombination == null,
                !connection.CanStart, resource != null && resource.CanGenerate && pendingCombination == null,
