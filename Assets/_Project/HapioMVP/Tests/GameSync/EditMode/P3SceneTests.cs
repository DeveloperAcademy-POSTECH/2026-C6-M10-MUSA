...
namespace C6.Prototype.GameSync.Tests
{
    ...
    public sealed class P3SceneTests
    {
        private const string ScenePath = "Assets/_Project/HapioMVP/Scenes/FivePlayerBattle.unity";
        private const string PreviousPath = "Assets/_Project/HapioMVP/Scenes/ThrowBattle.unity";
        ...
        private Scene scene;
        [SetUp] public void OpenSavedP3Scene()
        {
            Assert.That(AssetDatabase.LoadAssetAtPath<SceneAsset>(ScenePath), Is.Not.Null,
                "Prepare P3 FivePlayerBattle before verifying saved wiring.");
        ...
        [Test]
        public void EveryP3ServiceUsesOneSharedRootAndOptInCapacityFive()
        {
            ...
            Assert.That(Components<P3MultiplayerProbe>(scene).Single().gameObject, Is.SameAs(root));
            Assert.That(Components<P2ThrowProbe>(scene), Is.Empty);
        }
        [Test]
        public void CopiedP3CamerasMonsterFloorAndEverySceneReferenceAreLocal()
        {
            var layout = Components<SplitScreenLayout>(scene).Single();
        ...
        [Test]
        public void PreviousP2SceneKeepsItsTwoPlayerIdentityReleaseThrowsAndProbe()
        {
            ...
            try
            {
                ...
                Assert.That(game.BuildIdentifier, Is.EqualTo("22"));
                Assert.That(lobby.BuildIdentifier, Is.EqualTo("22"));
                Assert.That(game.MaximumParticipants, Is.EqualTo(2));
                Assert.That(lobby.MaximumParticipants, Is.EqualTo(2));
                Assert.That(lobby.ProtocolVersion, Is.EqualTo(10));
                Assert.That(controller.MaximumParticipants, Is.EqualTo(2));
                Assert.That(controller.OrbPhysicsEnabled && controller.ReleaseThrowsEnabled, Is.True);
                Assert.That(Components<P2ThrowProbe>(prior), Has.Length.EqualTo(1));
                Assert.That(Components<P3MultiplayerProbe>(prior), Is.Empty);
