...
{
    public sealed class P3LobbyTests
    {
        ...
        {
            authority = new LobbyAuthority(Room, Session, "23", "{\"schema\":2}", 789, 5);
            sequences.Clear(); nonces.Clear();
        }
        private LobbyHello Hello(ulong id)
        {
            if (!nonces.ContainsKey(id)) nonces[id] = Guid.NewGuid().ToString("N");
            return new LobbyHello { protocol = 23, build = "23", roomId = Room, clientNonce = nonces[id] };
        }
        ...
        private LobbyRequest Request(ulong id, string kind, bool ready = true)
        {
            ...
            return new LobbyRequest { protocol = 23, roomId = Room, sessionId = Session, sequence = sequence,
        ...
        public void LobbyLeaveCompactsSeatsKeepsConfigAcknowledgementAndResetsEveryReady()
        {
            ...
            Assert.That(LobbyWire.AcceptsSnapshot(before, after, "23", ""), Is.True);
        ...
        public void ChangedFrozenStartIdsAndReorderingSurvivorsAreRejected()
        {
            ...
            Assert.That(LobbyWire.AcceptsSnapshot(before, reordered, "23", ""), Is.False);
        ...
        public void ReadyReceiptAfterAnotherPlayersReadyResolvesAgainstCurrentRoster()
        {
            ...
            Assert.That(LobbyWire.AcceptsReceipt(current, receipt, 1, "23", nonces[1], Room, ready.revision), Is.True);
        }
        [Test]
        public void LegacyProtocolCannotEnterP3OrCarryP3Roster()
        {
            ...
            var legacy = new LobbyAuthority(Room, Session, "23", "{}", 1).Snapshot();
        ...
        public void BonjourP3CountRoundTripsAndSixIsRejected(int count)
        {
            var room = new RoomAdvertisement { RoomId = Room, Name = "C6", ProtocolVersion = 23, Build = "23",
