    ...
    public sealed class LobbyHostConfig
    {
        ...
        public static LobbyHostConfig Capture(ScreenLayoutConfig c)
        {
            ...
            return new LobbyHostConfig { upperFraction=c.UpperFraction, combinationRadius=c.CombinationRadiusFraction,
                ...
                monsterHp=c.MonsterMaxHp,monsterHp2=c.MonsterMaxHp2Players, monsterHp3=c.MonsterMaxHp3Players, monsterHp4=c.MonsterMaxHp4Players, monsterHp5=c.MonsterMaxHp5Players, monsterAttackFirstDelay=c.MonsterAttackFirstDelaySeconds, monsterAttackInterval=c.MonsterAttackIntervalSeconds, monsterAttackWarning=c.MonsterAttackWarningSeconds, defenseHold=c.DefenseHoldSeconds, defenseFailPenalty=c.DefenseFailPenaltySeconds, damage=c.BaseDamage, storageLimit=c.OrbStorageLimit,
        ...
        private static bool ValidValues(LobbyHostConfig value)
        {
            ...
            return In(value.upperFraction, .01f, .99f) && In(value.combinationRadius, .01f, .5f)
                ...
                && value.monsterHp >= 1 && value.monsterHp <= 100000 && value.damage >= 1 && value.damage <= 100000 && In(value.monsterHp2, 1, 100000) && In(value.monsterHp3, 1, 100000) && In(value.monsterHp4, 1, 100000) && In(value.monsterHp5, 1, 100000) && In(value.monsterAttackFirstDelay, 1, 600) && In(value.monsterAttackInterval, 1, 600) && In(value.monsterAttackWarning, .5f, 30)
