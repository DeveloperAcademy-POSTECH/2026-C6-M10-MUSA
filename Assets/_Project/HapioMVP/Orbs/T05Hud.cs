...
{
    /// <summary>Test fixture controls and input diagnostics; battle statistics remain unconnected.</summary>
    [DisallowMultipleComponent]
    [RequireComponent(typeof(SplitScreenLayout))]
    public sealed class T05Hud : MonoBehaviour
    {
        ...
        private string actionStatus = "Start a dev host";
        private string detailStatus = "3 fixed orbs / Actions reserve only";
        ...
        public void SetControls(bool allowHost, bool allowReset, bool allowEnd)
        {
            canHost = allowHost;
            canReset = allowReset;
            canEnd = allowEnd;
        ...
        private void Awake()
        {
            ...
            SetControls(canHost, canReset, canEnd);
        }
        ...
        {
            var canvasRect = Rect("T05Overlay", transform);
            ...
            {
                var events = new GameObject("T05EventSystem", typeof(EventSystem), typeof(InputSystemUIInputModule));
        ...
        private void CreateUpperHud()
        {
            ...
            var preview = Text("PreviewCaption", header, "T05  /  DEV INPUT", 10, Muted, TextAnchor.LowerLeft);
            ...
            var marker = Text("UnconnectedMarker", topContent, "FIXTURE TEST MODE  /  ACTIONS ONLY", 10, Muted, TextAnchor.MiddleCenter);
        ...
        private void CreateLowerHud()
        {
            ...
            Top(ActionLabel.rectTransform, 20f, 20f, 10f, 25f);
            DetailLabel = Text("ActionDetail", footer, detailStatus, 10, Muted, TextAnchor.UpperLeft);
            Top(DetailLabel.rectTransform, 20f, 20f, 39f, 33f);

            var buttons = Rect("FixtureButtons", footer);
            Bottom(buttons, 20f, 20f, 16f, 43f);
            HostButton = CreateButton("DevHostButton", buttons, "DEV HOST", 0f, 1f / 3f);
            ResetButton = CreateButton("ResetFixtureButton", buttons, "RESET FIXTURE", 1f / 3f, 2f / 3f);
            EndButton = CreateButton("EndTestButton", buttons, "END TEST", 2f / 3f, 1f);
        }
