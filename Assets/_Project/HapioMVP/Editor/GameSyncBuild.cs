...
namespace C6.Editor
{
    public static class GameSyncBuild
    {
        public const string ScenePath="Assets/_Project/HapioMVP/Scenes/TwoPlayerBattle.unity";
        ...
        [MenuItem("C6/T10-B/Prepare Two Player Battle")]
        public static void Prepare()
        {
            ...
            PlayerSettings.SetApplicationIdentifier(NamedBuildTarget.Standalone,"com.wolfuraark.c6prototype.t10b.desktop");
            PlayerSettings.SetApplicationIdentifier(NamedBuildTarget.iOS,"com.wolfuraark.c6prototype");
            PlayerSettings.iOS.buildNumber=T10GameSession.Build;
            ...
            AssetDatabase.SaveAssets();Debug.Log("C6_T10B_PREPARED scene="+ScenePath+" config="+ConfigPath+" build="+T10GameSession.Build);
        }
        static void CreateDerivedScene()
        {
            ...
            try
            {
                ...
                preview = EditorSceneManager.OpenPreviewScene(BattleLoopBuild.ScenePath);
                ...
                if (config == null || material == null)
                    throw new InvalidOperationException("T10B shared assets could not be reloaded after scene creation.");

                var layout = layouts[0];
                layout.gameObject.name = "T10TwoPlayerBattle";
                ...
                layout.gameObject.AddComponent<GameRuntimeConfig>();
                layout.gameObject.AddComponent<T10LobbySession>().Configure(config);
                layout.gameObject.AddComponent<T10LobbyHud>();
                layout.gameObject.AddComponent<T10LobbyController>();
                layout.gameObject.AddComponent<T10GameSession>();
                ...
                if (!EditorSceneManager.SaveScene(target, ScenePath))
                    throw new InvalidOperationException("Could not save the separate T10B scene.");
            }
            finally
            {
        ...
        public static void ApplyBonjour(BuildTarget target,string output)
        {
            ...
            Debug.Log("C6_T10B_BONJOUR_PLIST_APPLIED native=libSystem");
        }
        ...
        [MenuItem("C6/T10B/Export iOS")]
        public static void ExportIOS() => Build(BuildTarget.iOS, "iOS");

        [MenuItem("C6/T10B/Build macOS Two Player Battle")]
        public static void BuildMac() => Build(BuildTarget.StandaloneOSX, "macOS/C6Game.app");
        ...
        static void Build(BuildTarget target, string relativeOutput)
        {
            ...
            if (EditorUserBuildSettings.activeBuildTarget != target)
                throw new InvalidOperationException("Active build target is " + EditorUserBuildSettings.activeBuildTarget +
                    " but T10B requested " + target + ". Select the matching platform, or restart batch Unity with -buildTarget " + target + ".");
            string root = Path.GetFullPath(Path.Combine(Application.dataPath, ".."));
            string outputRoot = Environment.GetEnvironmentVariable("C6_T10B_OUTPUT_ROOT");
            if (string.IsNullOrWhiteSpace(outputRoot)) outputRoot = Path.Combine(root, "Builds", "T10-B");
            ...
            if (File.Exists(path) || Directory.Exists(path) && Directory.EnumerateFileSystemEntries(path).Any())
                throw new InvalidOperationException("Output exists; use a new C6_T10B_OUTPUT_ROOT to preserve previous builds.");
            Prepare();
            ...
            try
            {
                ...
                if (report.summary.result != BuildResult.Succeeded)
                    throw new InvalidOperationException("T10B build failed: " + report.summary.result);
                ...
                Debug.Log("C6_T10B_BUILD_COMPLETE target=" + target + " output=" + path);
            }
            ...
            {
                string logs = Path.Combine(root, "Logs", "T10-B");
        ...
        {
            public string buildId = "C6-T10B";
            public string buildNumber = T10GameSession.Build;
