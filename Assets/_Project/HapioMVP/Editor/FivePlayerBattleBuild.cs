...
namespace C6.Editor
{
    public static class FivePlayerBattleBuild
    {
        public const string BuildNumber = "23";
        public const string SourceScenePath = ThrowBattleBuild.ScenePath;
        public const string ScenePath="Assets/_Project/HapioMVP/Scenes/FivePlayerBattle.unity";
        ...
        [MenuItem("C6/Next Phase/P3/Connect Explicit Mac Validation Probe")]
        public static void EnsureProbeForValidation()
        {
            ...
            if (!opened && target.isDirty) throw new InvalidOperationException("Save the user's dirty P3 scene before connecting its probe.");
            ...
            try
            {
                ...
                if (games.Length != 1) throw new InvalidOperationException("One P3 game root is required.");
                var probes = Components<P3MultiplayerProbe>(target);
                ...
                if (probes.Length == 0)
                {
                    games[0].gameObject.AddComponent<P3MultiplayerProbe>();
                    VerifySceneReferences(target);
                    if (!EditorSceneManager.SaveScene(target)) throw new InvalidOperationException("Could not save the explicit P3 probe connection.");
                }
                Debug.Log("C6_P3_PROBE_CONNECTED scene=" + ScenePath + " mode=explicit-development-standalone-arguments-only");
            }
            finally
            {
        ...
        [MenuItem("C6/Next Phase/P3/Prepare Five Player Battle")]
        public static void Prepare()
        {
            ...
            if (prefab == null) throw new InvalidOperationException("Prepare the existing P1 monster prefab first; P3 does not replace it.");
            ...
            PlayerSettings.SetApplicationIdentifier(NamedBuildTarget.Standalone,"com.wolfuraark.c6prototype.p3.desktop");
            ...
            Debug.Log("C6_P3_PREPARED scene="+ScenePath+" config="+ConfigPath+" build="+BuildNumber);
        }
        static void CreateDerivedScene()
        {
            ...
            try
            {
                ...
                if (config == null || material == null)
                    throw new InvalidOperationException("P3 shared assets could not be reloaded after scene creation.");

                var layout = layouts[0];
                layout.gameObject.name = "P3ThrowBattle";
                ...
                if (monsters.Length != 1) throw new InvalidOperationException("P3 requires the single P1 benchmark monster.");
                ...
                lobby.Configure(config); lobby.ConfigureBuild(BuildNumber); lobby.ConfigureCapacity(5);
                lobbyController.Configure(lobby, lobbyHud);
                if (game.BuildIdentifier != ThrowBattleBuild.BuildNumber || !game.TransfersEnabled || !game.ReachableEdgeTransferDistance || !game.InterruptionHandlingEnabled)
                    throw new InvalidOperationException("P3 must derive from the preserved build22 release throw scene.");
                ...
                game.ConfigureMaximumParticipants(5);
                controller.ConfigureMaximumParticipants(5);
                var framing = layout.GetComponent<ThrowBattleFraming>();
                if (framing == null) throw new InvalidOperationException("P3 must preserve the saved P2 framing.");
                framing.Configure(layout, hud, monster);
                foreach (var oldProbe in Components<P2ThrowProbe>(target)) UnityEngine.Object.DestroyImmediate(oldProbe);
                layout.gameObject.AddComponent<P3MultiplayerProbe>();
                var floors = Components<ThrowBattleFloor>(target);
                if (floors.Length != 1) throw new InvalidOperationException("P3 must preserve the saved P2 floor.");
                floors[0].Configure(layout);
                ...
                if (!EditorSceneManager.SaveScene(target, ScenePath))
                    throw new InvalidOperationException("Could not save the separate P3 scene.");
            }
            finally
            {
        ...
        static void CreateThrowFloor(Scene scene, SplitScreenLayout layout)
        {
            ...
            var floor = new GameObject("P3ThrowFloor");
        ...
        static T RequireOnRoot<T>(GameObject[] roots, GameObject expectedRoot) where T : Component
        {
            ...
            if (found.Length != 1 || found[0].gameObject != expectedRoot)
                throw new InvalidOperationException("The saved P3 scene requires one " + typeof(T).Name + " on its shared game root.");
            return found[0];
        }
        ...
        public static void ValidateSavedScene()
        {
            ...
            try
            {
                if (!scene.IsValid() || !scene.isLoaded) throw new InvalidOperationException("Could not inspect the saved P3 scene.");
                var roots = scene.GetRootGameObjects();
                var layouts = roots.SelectMany(root => root.GetComponentsInChildren<SplitScreenLayout>(true)).ToArray();
                if (layouts.Length != 1 || layouts[0].Config != AssetDatabase.LoadAssetAtPath<ScreenLayoutConfig>(ConfigPath))
                    throw new InvalidOperationException("P3 must retain exactly one shared layout Config.");
                ...
                if (game.MaximumParticipants != 5 || lobby.MaximumParticipants != 5 || root.GetComponent<T09BattleController>().MaximumParticipants != 5 || !game.InterruptionHandlingEnabled || !game.TransfersEnabled || !game.ReachableEdgeTransferDistance || game.BuildIdentifier != BuildNumber || lobby.BuildIdentifier != BuildNumber)
                    throw new InvalidOperationException("The existing scene is not the expected build23 release throw mode; it was preserved.");
                if (layout.BattleCamera == null || layout.OrbCamera == null || layout.BattleCamera == layout.OrbCamera)
                    throw new InvalidOperationException("P3 requires its two distinct remapped cameras.");
                if (!RequireOnRoot<T09BattleController>(roots, root).OrbPhysicsEnabled
                    ...
                    throw new InvalidOperationException("The P3 scene must explicitly enable local orb physics and release throws.");
                ...
                if (floors.Length != 1 || floors[0].GetComponent<BoxCollider>() == null
                    ...
                    throw new InvalidOperationException("P3 requires one non-target bounce floor collider.");
                var monsters = Components<BenchmarkMonster>(scene);
                if (monsters.Length != 1 || Components<MonsterHitTarget>(scene).Length != 1)
                    throw new InvalidOperationException("The P3 scene must contain exactly one benchmark monster.");
        ...
        public static void ApplyBonjour(BuildTarget target,string output)
        {
            ...
            Debug.Log("C6_P3_BONJOUR_PLIST_APPLIED native=libSystem");
        }
        ...
        [MenuItem("C6/Next Phase/P3/Export iOS")]
        public static void ExportIOS() => Build(BuildTarget.iOS, "iOS");

        [MenuItem("C6/Next Phase/P3/Build macOS Five Player Battle")]
        public static void BuildMac() => Build(BuildTarget.StandaloneOSX, "macOS/C6FivePlayer.app");
        ...
        static void Build(BuildTarget target, string relativeOutput)
        {
            ...
            if (EditorUserBuildSettings.activeBuildTarget != target)
                throw new InvalidOperationException("Active build target is " + EditorUserBuildSettings.activeBuildTarget +
                    " but P3 requested " + target + ". Select the matching platform, or restart batch Unity with -buildTarget " + target + ".");
            string root = Path.GetFullPath(Path.Combine(Application.dataPath, ".."));
            string outputRoot = Environment.GetEnvironmentVariable("C6_P3_OUTPUT_ROOT");
            if (string.IsNullOrWhiteSpace(outputRoot)) outputRoot = Path.Combine(root, "Builds", "NextPhase", "P3");
            ...
            if (File.Exists(path) || Directory.Exists(path) && Directory.EnumerateFileSystemEntries(path).Any())
                throw new InvalidOperationException("Output exists; use a new C6_P3_OUTPUT_ROOT to preserve previous builds.");
            Prepare();
            ...
            try
            {
                ...
                if (report.summary.result != BuildResult.Succeeded)
                    throw new InvalidOperationException("P3 build failed: " + report.summary.result);
                ...
                if (target == BuildTarget.iOS)
                {
                    ...
                    if (lifecycle == null || lifecycle.status != "APPLIED_SOURCE_ONLY" || lifecycle.scene != ScenePath)
                        throw new InvalidOperationException("The lifecycle patch receipt must identify this actual P3 scene.");
                }
                receipt.outputValidation = "PASS";
                Debug.Log("C6_P3_BUILD_COMPLETE target=" + target + " output=" + path);
            }
            ...
            {
                string logs = Path.Combine(root, "Logs", "NextPhase", "P3");
