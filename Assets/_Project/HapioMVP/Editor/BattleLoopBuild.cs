...
namespace C6.Editor
{
    /// <summary>Builds a separate T09 battle loop scene while preserving the saved T08 scene.</summary>
    public static class BattleLoopBuild
    {
        public const string ScenePath = "Assets/_Project/HapioMVP/Scenes/BattleLoop.unity";
        public const string ConfigPath = "Assets/_Project/HapioMVP/Config/ScreenLayoutConfig.asset";
        public const string SourceScenePath = "Assets/_Project/HapioMVP/Scenes/CombinationSmoke.unity";
        ...
        [MenuItem("C6/T09/Prepare Battle Loop Scene")]
        public static void Prepare()
        {
            if (EditorApplication.isPlayingOrWillChangePlaymode)
                throw new InvalidOperationException("Stop Play mode before preparing T09.");
            if (AssetDatabase.LoadAssetAtPath<SceneAsset>(SourceScenePath) == null)
                throw new InvalidOperationException("T09 requires the existing saved T08 scene; it will not recreate or replace it.");
            var config = AssetDatabase.LoadAssetAtPath<ScreenLayoutConfig>(ConfigPath);
            if (config == null)
                throw new InvalidOperationException("T09 requires the existing shared ScreenLayoutConfig asset.");
            ...
            if (createScene && File.Exists(ScenePath))
                throw new InvalidOperationException("The T09 scene path is occupied by another or unreadable asset; preserving it.");
            if (createScene && !Application.isBatchMode)
                for (int i = 0; i < SceneManager.sceneCount; i++)
                    if (string.IsNullOrEmpty(SceneManager.GetSceneAt(i).path))
                        throw new InvalidOperationException("Save the open untitled scene or use a verification copy. T09 will not replace it.");
            ...
            PlayerSettings.SetApplicationIdentifier(NamedBuildTarget.Standalone, "com.wolfuraark.c6prototype.t09.desktop");
            PlayerSettings.iOS.buildNumber = "13";
            ...
            AssetDatabase.SaveAssets();
            Debug.Log("C6_T09_PREPARED scene=" + ScenePath + " config=" + ConfigPath + " build=13");
        }
        ...
        static void CreateDerivedScene()
        {
            ...
            try
            {
                // Preview loads the saved asset independently of any open, possibly dirty T08 scene.
                preview = EditorSceneManager.OpenPreviewScene(SourceScenePath);
                if (!preview.IsValid() || !preview.isLoaded)
                    throw new InvalidOperationException("Could not open the saved T08 scene as a preview.");
                ...
                if (sourceLayouts.Length != 1 || sourceLayouts[0].BattleCamera == null || sourceLayouts[0].OrbCamera == null)
                    throw new InvalidOperationException("The saved T08 scene must contain one wired SplitScreenLayout.");
                ...
                if (battleCameraName == orbCameraName)
                    throw new InvalidOperationException("T08 cameras need distinct names for safe scene reference remapping.");
                ...
                // Remove only copied T08 components, input first because it previously required the controller.
                ...
                foreach (var oldProbe in roots.SelectMany(root => root.GetComponentsInChildren<T08Probe>(true)).ToArray())
                    UnityEngine.Object.DestroyImmediate(oldProbe);
                foreach (var oldController in roots.SelectMany(root => root.GetComponentsInChildren<T08CombinationController>(true)).ToArray())
                    UnityEngine.Object.DestroyImmediate(oldController);
                foreach (var oldHud in roots.SelectMany(root => root.GetComponentsInChildren<T08Hud>(true)).ToArray())
                ...
                if (layouts.Length != 1 || battleCameras.Length != 1 || orbCameras.Length != 1 || cameras.Length != 2)
                    throw new InvalidOperationException("The copied T08 layout/cameras could not be remapped unambiguously.");
                var config = AssetDatabase.LoadAssetAtPath<ScreenLayoutConfig>(ConfigPath);
                var material = AssetDatabase.LoadAssetAtPath<Material>(SpriteMaterialPath);
                if (config == null || material == null)
                    throw new InvalidOperationException("T09 shared assets could not be reloaded after scene creation.");

                var layout = layouts[0];
                layout.gameObject.name = "T09BattleLoop";
                layout.Configure(config, battleCameras[0], orbCameras[0]);
                var hud = layout.gameObject.AddComponent<T09Hud>();
                hud.Configure(layout);
                var session = layout.gameObject.GetComponent<DirectConnectionSession>();
                if (session == null)
                    throw new InvalidOperationException("The saved T08 scene must contain its DirectConnectionSession.");
                var frames = roots.SelectMany(root => root.GetComponentsInChildren<AttackLaunchFrame>(true)).ToArray();
                if (frames.Length != 1) throw new InvalidOperationException("The saved T08 scene needs one launch frame.");
                var launchFrame = frames[0];
                launchFrame.Configure(config);

                // Preserve the copied T08 collider and its tuning; only remap the scene reference.
                ...
                if (hitTargets.Length != 1 || hitTargets[0].GetComponent<BoxCollider>() == null)
                    throw new InvalidOperationException("The saved T08 scene needs one wired training hit target.");
                var hitTarget = hitTargets[0];

                var controller = layout.gameObject.AddComponent<T09BattleController>();
                ...
                // New scene render settings belong to the new scene, not to the preview or open T08.
                RenderSettings.ambientMode = AmbientMode.Flat;
                RenderSettings.ambientLight = new Color(.37f, .42f, .49f);
                RenderSettings.fog = false;
                if (!EditorSceneManager.SaveScene(target, ScenePath))
                    throw new InvalidOperationException("Could not save the separate T09 scene.");
            }
            finally
            {
        ...
        [MenuItem("C6/T09/Export iOS")]
        public static void ExportIOS() => Build(BuildTarget.iOS, "iOS");

        [MenuItem("C6/T09/Build macOS Battle Loop")]
        public static void BuildMac() => Build(BuildTarget.StandaloneOSX, "macOS/C6Battle.app");
        ...
        static void Build(BuildTarget target, string relativeOutput)
        {
            ...
            if (EditorUserBuildSettings.activeBuildTarget != target)
                throw new InvalidOperationException("Active build target is " + EditorUserBuildSettings.activeBuildTarget +
                    " but T09 requested " + target + ". Select the matching platform, or restart batch Unity with -buildTarget " + target + ".");
            string root = Path.GetFullPath(Path.Combine(Application.dataPath, ".."));
            string outputRoot = Environment.GetEnvironmentVariable("C6_T09_OUTPUT_ROOT");
            if (string.IsNullOrWhiteSpace(outputRoot)) outputRoot = Path.Combine(root, "Builds", "T09");
            ...
            if (File.Exists(path) || Directory.Exists(path) && Directory.EnumerateFileSystemEntries(path).Any())
                throw new InvalidOperationException("Output exists; use a new C6_T09_OUTPUT_ROOT to preserve previous builds.");
            Prepare();
            ...
            try
            {
                ...
                if (report.summary.result != BuildResult.Succeeded)
                    throw new InvalidOperationException("T09 build failed: " + report.summary.result);
                ...
                Debug.Log("C6_T09_BUILD_COMPLETE target=" + target + " output=" + path);
            }
            ...
            {
                string logs = Path.Combine(root, "Logs", "T09");
