...
namespace C6.Editor
{
    /// <summary>Builds a separate T07 resource scene while preserving the saved T06 scene.</summary>
    public static class ResourceSmokeBuild
    {
        public const string ScenePath = "Assets/_Project/HapioMVP/Scenes/ResourceSmoke.unity";
        public const string ConfigPath = "Assets/_Project/HapioMVP/Config/ScreenLayoutConfig.asset";
        public const string SourceScenePath = "Assets/_Project/HapioMVP/Scenes/AttackSmoke.unity";
        ...
        [MenuItem("C6/T07/Prepare Resource Scene")]
        public static void Prepare()
        {
            if (EditorApplication.isPlayingOrWillChangePlaymode)
                throw new InvalidOperationException("Stop Play mode before preparing T07.");
            if (AssetDatabase.LoadAssetAtPath<SceneAsset>(SourceScenePath) == null)
                throw new InvalidOperationException("T07 requires the existing saved T06 scene; it will not recreate or replace it.");
            var config = AssetDatabase.LoadAssetAtPath<ScreenLayoutConfig>(ConfigPath);
            if (config == null)
                throw new InvalidOperationException("T07 requires the existing shared ScreenLayoutConfig asset.");
            ...
            if (createScene && File.Exists(ScenePath))
                throw new InvalidOperationException("The T07 scene path is occupied by another or unreadable asset; preserving it.");
            if (createScene && !Application.isBatchMode)
                for (int i = 0; i < SceneManager.sceneCount; i++)
                    if (string.IsNullOrEmpty(SceneManager.GetSceneAt(i).path))
                        throw new InvalidOperationException("Save the open untitled scene or use a verification copy. T07 will not replace it.");
            ...
            PlayerSettings.SetApplicationIdentifier(NamedBuildTarget.Standalone, "com.wolfuraark.c6prototype.t07.desktop");
            ...
            AssetDatabase.SaveAssets();
            Debug.Log("C6_T07_PREPARED scene=" + ScenePath + " config=" + ConfigPath + " build=9");
        }
        ...
        static void CreateDerivedScene()
        {
            ...
            try
            {
                // Preview loads the saved asset independently of any open, possibly dirty T06 scene.
                preview = EditorSceneManager.OpenPreviewScene(SourceScenePath);
                if (!preview.IsValid() || !preview.isLoaded)
                    throw new InvalidOperationException("Could not open the saved T06 scene as a preview.");
                ...
                if (sourceLayouts.Length != 1 || sourceLayouts[0].BattleCamera == null || sourceLayouts[0].OrbCamera == null)
                    throw new InvalidOperationException("The saved T06 scene must contain one wired SplitScreenLayout.");
                ...
                if (battleCameraName == orbCameraName)
                    throw new InvalidOperationException("T06 cameras need distinct names for safe scene reference remapping.");
                ...
                // Remove only copied T06 components, input first because it previously required the controller.
                ...
                foreach (var oldProbe in roots.SelectMany(root => root.GetComponentsInChildren<T06Probe>(true)).ToArray())
                    UnityEngine.Object.DestroyImmediate(oldProbe);
                foreach (var oldController in roots.SelectMany(root => root.GetComponentsInChildren<T06AttackController>(true)).ToArray())
                    UnityEngine.Object.DestroyImmediate(oldController);
                foreach (var oldHud in roots.SelectMany(root => root.GetComponentsInChildren<T06Hud>(true)).ToArray())
                ...
                if (layouts.Length != 1 || battleCameras.Length != 1 || orbCameras.Length != 1 || cameras.Length != 2)
                    throw new InvalidOperationException("The copied T06 layout/cameras could not be remapped unambiguously.");
                var config = AssetDatabase.LoadAssetAtPath<ScreenLayoutConfig>(ConfigPath);
                var material = AssetDatabase.LoadAssetAtPath<Material>(SpriteMaterialPath);
                if (config == null || material == null)
                    throw new InvalidOperationException("T07 shared assets could not be reloaded after scene creation.");

                var layout = layouts[0];
                layout.gameObject.name = "T07ResourceSmoke";
                layout.Configure(config, battleCameras[0], orbCameras[0]);
                var hud = layout.gameObject.AddComponent<T07Hud>();
                hud.Configure(layout);
                var session = layout.gameObject.GetComponent<DirectConnectionSession>();
                if (session == null)
                    throw new InvalidOperationException("The saved T06 scene must contain its DirectConnectionSession.");
                var frames = roots.SelectMany(root => root.GetComponentsInChildren<AttackLaunchFrame>(true)).ToArray();
                if (frames.Length != 1) throw new InvalidOperationException("The saved T06 scene needs one launch frame.");
                var launchFrame = frames[0];
                launchFrame.Configure(config);

                // Preserve the copied T06 collider and its tuning; only remap the scene reference.
                ...
                if (hitTargets.Length != 1 || hitTargets[0].GetComponent<BoxCollider>() == null)
                    throw new InvalidOperationException("The saved T06 scene needs one wired training hit target.");
                var hitTarget = hitTargets[0];

                var controller = layout.gameObject.AddComponent<T07ResourceController>();
                ...
                // New scene render settings belong to the new scene, not to the preview or open T06.
                RenderSettings.ambientMode = AmbientMode.Flat;
                RenderSettings.ambientLight = new Color(.37f, .42f, .49f);
                RenderSettings.fog = false;
                if (!EditorSceneManager.SaveScene(target, ScenePath))
                    throw new InvalidOperationException("Could not save the separate T07 scene.");
            }
            finally
            {
        ...
        [MenuItem("C6/T07/Export iOS")]
        public static void ExportIOS() => Build(BuildTarget.iOS, "iOS");

        [MenuItem("C6/T07/Build macOS Resources")]
        public static void BuildMac() => Build(BuildTarget.StandaloneOSX, "macOS/C6Resources.app");
        ...
        static void Build(BuildTarget target, string relativeOutput)
        {
            ...
            if (EditorUserBuildSettings.activeBuildTarget != target)
                throw new InvalidOperationException("Active build target is " + EditorUserBuildSettings.activeBuildTarget +
                    " but T07 requested " + target + ". Select the matching platform, or restart batch Unity with -buildTarget " + target + ".");
            string root = Path.GetFullPath(Path.Combine(Application.dataPath, ".."));
            string outputRoot = Environment.GetEnvironmentVariable("C6_T07_OUTPUT_ROOT");
            if (string.IsNullOrWhiteSpace(outputRoot)) outputRoot = Path.Combine(root, "Builds", "T07");
            ...
            if (File.Exists(path) || Directory.Exists(path) && Directory.EnumerateFileSystemEntries(path).Any())
                throw new InvalidOperationException("Output exists; use a new C6_T07_OUTPUT_ROOT to preserve previous builds.");
            Prepare();
            ...
            try
            {
                ...
                if (report.summary.result != BuildResult.Succeeded)
                    throw new InvalidOperationException("T07 build failed: " + report.summary.result);
                ...
                Debug.Log("C6_T07_BUILD_COMPLETE target=" + target + " output=" + path);
            }
            ...
            {
                string logs = Path.Combine(root, "Logs", "T07");
        ...
        {
            public string buildId = "C6-T07";
            public string buildNumber = "9";
