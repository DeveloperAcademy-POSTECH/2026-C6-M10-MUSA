...
namespace C6.Editor
{
    /// <summary>Builds a separate T06 physics fixture scene while preserving the saved T05 scene.</summary>
    public static class AttackSmokeBuild
    {
        public const string ScenePath = "Assets/_Project/HapioMVP/Scenes/AttackSmoke.unity";
        public const string ConfigPath = "Assets/_Project/HapioMVP/Config/ScreenLayoutConfig.asset";
        public const string SourceScenePath = "Assets/_Project/HapioMVP/Scenes/OrbInputSmoke.unity";
        ...
        [MenuItem("C6/T06/Prepare Attack Scene")]
        public static void Prepare()
        {
            if (EditorApplication.isPlayingOrWillChangePlaymode)
                throw new InvalidOperationException("Stop Play mode before preparing T06.");
            if (AssetDatabase.LoadAssetAtPath<SceneAsset>(SourceScenePath) == null)
                throw new InvalidOperationException("T06 requires the existing saved T05 scene; it will not recreate or replace it.");
            var config = AssetDatabase.LoadAssetAtPath<ScreenLayoutConfig>(ConfigPath);
            if (config == null)
                throw new InvalidOperationException("T06 requires the existing shared ScreenLayoutConfig asset.");
            ...
            if (createScene && File.Exists(ScenePath))
                throw new InvalidOperationException("The T06 scene path is occupied by another or unreadable asset; preserving it.");
            if (createScene && !Application.isBatchMode)
                for (int i = 0; i < SceneManager.sceneCount; i++)
                    if (string.IsNullOrEmpty(SceneManager.GetSceneAt(i).path))
                        throw new InvalidOperationException("Save the open untitled scene or use a verification copy. T06 will not replace it.");
            ...
            PlayerSettings.SetApplicationIdentifier(NamedBuildTarget.Standalone, "com.wolfuraark.c6prototype.t06.desktop");
            ...
            AssetDatabase.SaveAssets();
            Debug.Log("C6_T06_PREPARED scene=" + ScenePath + " config=" + ConfigPath + " build=8");
        }
        ...
        static void CreateDerivedScene()
        {
            ...
            try
            {
                // Preview loads the saved asset independently of any open, possibly dirty T05 scene.
                preview = EditorSceneManager.OpenPreviewScene(SourceScenePath);
                if (!preview.IsValid() || !preview.isLoaded)
                    throw new InvalidOperationException("Could not open the saved T05 scene as a preview.");
                ...
                if (sourceLayouts.Length != 1 || sourceLayouts[0].BattleCamera == null || sourceLayouts[0].OrbCamera == null)
                    throw new InvalidOperationException("The saved T05 scene must contain one wired SplitScreenLayout.");
                ...
                if (battleCameraName == orbCameraName)
                    throw new InvalidOperationException("T05 cameras need distinct names for safe scene reference remapping.");
                ...
                // Remove only copied T05 components, input first because it previously required the controller.
                ...
                foreach (var oldProbe in roots.SelectMany(root => root.GetComponentsInChildren<T05Probe>(true)).ToArray())
                    UnityEngine.Object.DestroyImmediate(oldProbe);
                foreach (var oldController in roots.SelectMany(root => root.GetComponentsInChildren<T05OrbController>(true)).ToArray())
                    UnityEngine.Object.DestroyImmediate(oldController);
                foreach (var oldHud in roots.SelectMany(root => root.GetComponentsInChildren<T05Hud>(true)).ToArray())
                ...
                if (layouts.Length != 1 || battleCameras.Length != 1 || orbCameras.Length != 1 || cameras.Length != 2)
                    throw new InvalidOperationException("The copied T05 layout/cameras could not be remapped unambiguously.");
                var config = AssetDatabase.LoadAssetAtPath<ScreenLayoutConfig>(ConfigPath);
                var material = AssetDatabase.LoadAssetAtPath<Material>(SpriteMaterialPath);
                if (config == null || material == null)
                    throw new InvalidOperationException("T06 shared assets could not be reloaded after scene creation.");

                var layout = layouts[0];
                layout.gameObject.name = "T06AttackSmoke";
                layout.Configure(config, battleCameras[0], orbCameras[0]);
                var hud = layout.gameObject.AddComponent<T06Hud>();
                hud.Configure(layout);
                var session = layout.gameObject.GetComponent<DirectConnectionSession>();
                if (session == null)
                    throw new InvalidOperationException("The saved T05 scene must contain its DirectConnectionSession.");
                var launchFrame = layout.gameObject.AddComponent<AttackLaunchFrame>();
                launchFrame.Configure(config);

                // The copied T05 dummy remains visual geometry. This one separate, static collider is the T06 hit target.
                ...
                var targetObject = new GameObject("T06 Fixed Training Hit Target");
                targetObject.layer = battleLayer;
                var hitCollider = targetObject.AddComponent<BoxCollider>();
                hitCollider.center = new Vector3(0f, 1.4f, 0f);
                hitCollider.size = new Vector3(1.2f, 2.6f, .65f);
                hitCollider.isTrigger = false;
                var hitTarget = targetObject.AddComponent<MonsterHitTarget>();
                hitTarget.Configure("dev-training-dummy");

                var controller = layout.gameObject.AddComponent<T06AttackController>();
                ...
                // New scene render settings belong to the new scene, not to the preview or open T05.
                RenderSettings.ambientMode = AmbientMode.Flat;
                RenderSettings.ambientLight = new Color(.37f, .42f, .49f);
                RenderSettings.fog = false;
                if (!EditorSceneManager.SaveScene(target, ScenePath))
                    throw new InvalidOperationException("Could not save the separate T06 scene.");
            }
            finally
            {
        ...
        [MenuItem("C6/T06/Export iOS")]
        public static void ExportIOS() => Build(BuildTarget.iOS, "iOS");

        [MenuItem("C6/T06/Build macOS Attack")]
        public static void BuildMac() => Build(BuildTarget.StandaloneOSX, "macOS/C6Attack.app");
        ...
        static void Build(BuildTarget target, string relativeOutput)
        {
            ...
            if (EditorUserBuildSettings.activeBuildTarget != target)
                throw new InvalidOperationException("Active build target is " + EditorUserBuildSettings.activeBuildTarget +
                    " but T06 requested " + target + ". Select the matching platform, or restart batch Unity with -buildTarget " + target + ".");
            string root = Path.GetFullPath(Path.Combine(Application.dataPath, ".."));
            string outputRoot = Environment.GetEnvironmentVariable("C6_T06_OUTPUT_ROOT");
            if (string.IsNullOrWhiteSpace(outputRoot)) outputRoot = Path.Combine(root, "Builds", "T06");
            ...
            if (File.Exists(path) || Directory.Exists(path) && Directory.EnumerateFileSystemEntries(path).Any())
                throw new InvalidOperationException("Output exists; use a new C6_T06_OUTPUT_ROOT to preserve previous builds.");
            Prepare();
            ...
            try
            {
                ...
                if (report.summary.result != BuildResult.Succeeded)
                    throw new InvalidOperationException("T06 build failed: " + report.summary.result);
                ...
                Debug.Log("C6_T06_BUILD_COMPLETE target=" + target + " output=" + path);
            }
            ...
            {
                string logs = Path.Combine(root, "Logs", "T06");
        ...
        {
            public string buildId = "C6-T06";
            public string buildNumber = "8";
