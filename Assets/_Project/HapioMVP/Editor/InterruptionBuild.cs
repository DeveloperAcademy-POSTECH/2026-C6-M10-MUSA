...
namespace C6.Editor
{
    public static class InterruptionBuild
    {
        public const string BuildNumber = "20";
        public const string SourceScenePath = IntegratedDeviceBuild.ScenePath;
        public const string ScenePath="Assets/_Project/HapioMVP/Scenes/InterruptionBattle.unity";
        ...
        [MenuItem("C6/T13/Prepare Interruption Battle")]
        public static void Prepare()
        {
            ...
            PlayerSettings.SetApplicationIdentifier(NamedBuildTarget.Standalone,"com.wolfuraark.c6prototype.t13.desktop");
            ...
            AssetDatabase.SaveAssets();Debug.Log("C6_T13_PREPARED scene="+ScenePath+" config="+ConfigPath+" build="+BuildNumber);
        }
        static void CreateDerivedScene()
        {
            ...
            try
            {
                ...
                if (config == null || material == null)
                    throw new InvalidOperationException("T13 shared assets could not be reloaded after scene creation.");

                var layout = layouts[0];
                layout.gameObject.name = "T13InterruptionBattle";
                ...
                // The saved T12 scene already owns these services. Reuse exactly one of each;
                ...
                if (game.BuildIdentifier != IntegratedDeviceBuild.BuildNumber || !game.TransfersEnabled || !game.ReachableEdgeTransferDistance || game.InterruptionHandlingEnabled)
                    throw new InvalidOperationException("T13 must derive from the preserved build19 integrated scene.");
                ...
                if (!EditorSceneManager.SaveScene(target, ScenePath))
                    throw new InvalidOperationException("Could not save the separate T13 scene.");
            }
            finally
            {
        ...
        static T RequireOnRoot<T>(GameObject[] roots, GameObject expectedRoot) where T : Component
        {
            ...
            if (found.Length != 1 || found[0].gameObject != expectedRoot)
                throw new InvalidOperationException("The saved T13 scene requires one " + typeof(T).Name + " on its shared game root.");
            return found[0];
        }
        ...
        static void ValidateSavedScene()
        {
            ...
            try
            {
                if (!scene.IsValid() || !scene.isLoaded) throw new InvalidOperationException("Could not inspect the saved T13 scene.");
                var roots = scene.GetRootGameObjects();
                var layouts = roots.SelectMany(root => root.GetComponentsInChildren<SplitScreenLayout>(true)).ToArray();
                if (layouts.Length != 1 || layouts[0].Config != AssetDatabase.LoadAssetAtPath<ScreenLayoutConfig>(ConfigPath))
                    throw new InvalidOperationException("T13 must retain exactly one shared layout Config.");
                ...
                if (!game.InterruptionHandlingEnabled || !game.TransfersEnabled || !game.ReachableEdgeTransferDistance || game.BuildIdentifier != BuildNumber || lobby.BuildIdentifier != BuildNumber)
                    throw new InvalidOperationException("The existing scene is not the expected build 20 interruption mode; it was preserved.");
                if (layout.BattleCamera == null || layout.OrbCamera == null || layout.BattleCamera == layout.OrbCamera)
                    throw new InvalidOperationException("T13 requires its two distinct remapped cameras.");
        ...
        public static void ApplyBonjour(BuildTarget target,string output)
        {
            ...
            Debug.Log("C6_T13_BONJOUR_PLIST_APPLIED native=libSystem");
        }
        ...
        [MenuItem("C6/T13/Export iOS")]
        public static void ExportIOS() => Build(BuildTarget.iOS, "iOS");

        [MenuItem("C6/T13/Build macOS Interruption Battle")]
        public static void BuildMac() => Build(BuildTarget.StandaloneOSX, "macOS/C6Interruption.app");
        ...
        static void Build(BuildTarget target, string relativeOutput)
        {
            ...
            if (EditorUserBuildSettings.activeBuildTarget != target)
                throw new InvalidOperationException("Active build target is " + EditorUserBuildSettings.activeBuildTarget +
                    " but T13 requested " + target + ". Select the matching platform, or restart batch Unity with -buildTarget " + target + ".");
            string root = Path.GetFullPath(Path.Combine(Application.dataPath, ".."));
            string outputRoot = Environment.GetEnvironmentVariable("C6_T13_OUTPUT_ROOT");
            if (string.IsNullOrWhiteSpace(outputRoot)) outputRoot = Path.Combine(root, "Builds", "T13");
            ...
            if (File.Exists(path) || Directory.Exists(path) && Directory.EnumerateFileSystemEntries(path).Any())
                throw new InvalidOperationException("Output exists; use a new C6_T13_OUTPUT_ROOT to preserve previous builds.");
            Prepare();
            ...
            try
            {
                ...
                if (report.summary.result != BuildResult.Succeeded)
                    throw new InvalidOperationException("T13 build failed: " + report.summary.result);
                ...
                if (target == BuildTarget.iOS)
                {
                    ...
                    if (lifecycle == null || lifecycle.status != "APPLIED_SOURCE_ONLY" || lifecycle.scene != ScenePath)
                        throw new InvalidOperationException("The lifecycle patch receipt must identify this actual T13 scene.");
                }
                receipt.outputValidation = "PASS";
                Debug.Log("C6_T13_BUILD_COMPLETE target=" + target + " output=" + path);
            }
            ...
            {
                string logs = Path.Combine(root, "Logs", "T13");
