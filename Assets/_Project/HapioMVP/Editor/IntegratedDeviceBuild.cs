...
namespace C6.Editor
{
    public static class IntegratedDeviceBuild
    {
        public const string BuildNumber = "19";
        public const string SourceScenePath = OrbTransferBuild.ScenePath;
        public const string ScenePath="Assets/_Project/HapioMVP/Scenes/IntegratedDeviceBattle.unity";
        ...
        [MenuItem("C6/T12/Prepare Integrated Device Battle")]
        public static void Prepare()
        {
            ...
            PlayerSettings.SetApplicationIdentifier(NamedBuildTarget.Standalone,"com.wolfuraark.c6prototype.t12.desktop");
            ...
            AssetDatabase.SaveAssets();Debug.Log("C6_T12_PREPARED scene="+ScenePath+" config="+ConfigPath+" build="+BuildNumber);
        }
        static void CreateDerivedScene()
        {
            ...
            try
            {
                ...
                if (config == null || material == null)
                    throw new InvalidOperationException("T12 shared assets could not be reloaded after scene creation.");

                var layout = layouts[0];
                layout.gameObject.name = "T12IntegratedDeviceBattle";
                ...
                // The saved T11 scene already owns these services. Reuse exactly one of each;
                ...
                if (!EditorSceneManager.SaveScene(target, ScenePath))
                    throw new InvalidOperationException("Could not save the separate T12 scene.");
            }
            finally
            {
        ...
        static T RequireOnRoot<T>(GameObject[] roots, GameObject expectedRoot) where T : Component
        {
            ...
            if (found.Length != 1 || found[0].gameObject != expectedRoot)
                throw new InvalidOperationException("The saved T12 scene requires one " + typeof(T).Name + " on its shared game root.");
            return found[0];
        }
        ...
        static void ValidateSavedScene()
        {
            ...
            try
            {
                if (!scene.IsValid() || !scene.isLoaded) throw new InvalidOperationException("Could not inspect the saved T12 scene.");
                var roots = scene.GetRootGameObjects();
                var layouts = roots.SelectMany(root => root.GetComponentsInChildren<SplitScreenLayout>(true)).ToArray();
                if (layouts.Length != 1 || layouts[0].Config != AssetDatabase.LoadAssetAtPath<ScreenLayoutConfig>(ConfigPath))
                    throw new InvalidOperationException("T12 must retain exactly one shared layout Config.");
                ...
                if (!game.TransfersEnabled || !game.ReachableEdgeTransferDistance || game.BuildIdentifier != BuildNumber || lobby.BuildIdentifier != BuildNumber)
                    throw new InvalidOperationException("The existing scene is not the expected build 19 integrated mode; it was preserved.");
                if (layout.BattleCamera == null || layout.OrbCamera == null || layout.BattleCamera == layout.OrbCamera)
                    throw new InvalidOperationException("T12 requires its two distinct remapped cameras.");
        ...
        public static void ApplyBonjour(BuildTarget target,string output)
        {
            ...
            Debug.Log("C6_T12_BONJOUR_PLIST_APPLIED native=libSystem");
        }
        ...
        [MenuItem("C6/T12/Export iOS")]
        public static void ExportIOS() => Build(BuildTarget.iOS, "iOS");

        [MenuItem("C6/T12/Build macOS Integrated Device Battle")]
        public static void BuildMac() => Build(BuildTarget.StandaloneOSX, "macOS/C6Integrated.app");
        ...
        static void Build(BuildTarget target, string relativeOutput)
        {
            ...
            if (EditorUserBuildSettings.activeBuildTarget != target)
                throw new InvalidOperationException("Active build target is " + EditorUserBuildSettings.activeBuildTarget +
                    " but T12 requested " + target + ". Select the matching platform, or restart batch Unity with -buildTarget " + target + ".");
            string root = Path.GetFullPath(Path.Combine(Application.dataPath, ".."));
            string outputRoot = Environment.GetEnvironmentVariable("C6_T12_OUTPUT_ROOT");
            if (string.IsNullOrWhiteSpace(outputRoot)) outputRoot = Path.Combine(root, "Builds", "T12");
            ...
            if (File.Exists(path) || Directory.Exists(path) && Directory.EnumerateFileSystemEntries(path).Any())
                throw new InvalidOperationException("Output exists; use a new C6_T12_OUTPUT_ROOT to preserve previous builds.");
            Prepare();
            ...
            try
            {
                ...
                if (report.summary.result != BuildResult.Succeeded)
                    throw new InvalidOperationException("T12 build failed: " + report.summary.result);
                ...
                Debug.Log("C6_T12_BUILD_COMPLETE target=" + target + " output=" + path);
            }
            ...
            {
                string logs = Path.Combine(root, "Logs", "T12");
